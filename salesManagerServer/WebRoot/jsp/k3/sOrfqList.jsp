...
function getCustomer(){ 
	var array = window.showModalDialog("<%=basePath %>/customer!customerOpenWindow", window, "dialogWidth:800px; dialogHeight:530px; status:no;help:no");
	//for chrome
    if (array == undefined) {
...
function addOrfq() {
	var returnValue = window.showModalDialog("<%=basePath %>/orfq!initSOrfq", window, 
...
function updateOrfq(interId) {
	var returnValue = window.showModalDialog("<%=basePath %>/orfq!updateSOrfq?orfq.fInterID=" + interId, 
			window, "dialogWidth:" + winWidth + "px; dialogHeight:" + winHeight + "; status:no;help:no;resizable:yes");
	//for chrome
	if (!returnValue) {
	    returnValue = window.returnValue;
...
function verifyOrfq(interId) {
	var returnValue = window.showModalDialog("<%=basePath %>/orfq!verify?orfq.fInterID=" + interId, 
			window, "dialogWidth:" + winWidth + "px; dialogHeight:" + winHeight + "; status:no;help:no;resizable:yes");
	//for chrome
	if (!returnValue) {
	    returnValue = window.returnValue;
...
<body class="ui-lv2bg">
<s:if test='#session.USER_INFO.k3UserId != null' >
	<!-- 标准工具栏 -->
	<nav class="navbar navbar-default crm-toolbar" role="navigation">
		<ul class="nav navbar-nav">
	    	<li class="active"><a href="javascript:addOrfq();"><span class="glyphicon glyphicon-star"></span>&nbsp;新增</a></li>
	    	<li class="active"><a href="orfq!index"><span class="glyphicon glyphicon-refresh"></span>&nbsp;清空查询条件</a></li>
	    </ul>
	</nav>
</s:if>
	  ...
	  <s:iterator value="orfqList" status="dl">
		<tr class="ui-table-style1-tr2"> 
		  <td><a href="#" title="<s:property value='customerName'/>" class="easyui-tooltip">
		  		<s:if test="customerName.length() > 6">
		  			<s:property value="customerName.substring(0,6)+' ...'"/>
		  		</s:if><s:else>
		  ...
		  <td><a href="javascript:showOrfq(<s:property value="fInterID"/>);"> 查看 </a> 

		  <s:if test="fStatus == 0">	
		  	<s:if test="roleId==3">	
			  <a href="javascript:verifyOrfq(<s:property value="fInterID"/>);"> 审核 </a>
			</s:if>
		  	<s:if test="roleId==2">	
			  <a href="javascript:verifyOrfq(<s:property value="fInterID"/>);"> 审核 </a>
			</s:if>
		  ...
		  <s:if test="fStatus == 0">	
			  <a href="<%=basePath %>orfq!deleteSOrfq?orfq.fInterID=<s:property value="fInterID"/>"> 删除 </a>
		  </s:if>
		  <s:if test="fStatus == 2">	
			  <a href="javascript:updateOrfq(<s:property value="fInterID"/>);"> 修改 </a>
			  <a href="<%=basePath %>orfq!deleteSOrfq?orfq.fInterID=<s:property value="fInterID"/>"> 删除 </a>
		  </s:if>
		  </td>
		</tr>
