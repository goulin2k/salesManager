...
<script type="text/javascript">
	...
	function deleteSearch() {	//清除查询条件
		window.location.href = basePath + '/report!salesInquiryReport';
	}
	...
	function pagesearch(){
		...
		var url= basePath+"report!salesInquiryReport?pageNumber="+pageNumber+"&startTime="+startTime+"&endTime="+endTime;
		url = encodeURI(url);
...
<body class="ui-lv2bg">
	...
	<nav class="navbar navbar-default" role="navigation">
	    ...
	    <form class="navbar-form navbar-right form-inline" role="search" 
	    	id="reportForm" name="reportForm" action="<%=basePath %>report!salesInquiryReport" method="post">
     ...
     <div class="panel-heading crm-table-title"><B>报表统计信息</B></div>
		<table class="table table-hover"  cellspacing="0" cellpadding="0" >
			  <tr class="ui-table-style1-tr2">
                <td>平均回复：${averageReplyDate} 小时</td>
                <td></td>
                <td>最长回复：${maxReplyDate } 小时</td>
                <td> </td>
                <td>最短回复：${minReplyDate } 小时</td>
               <td></td>
              </tr>
       </table>
    </div>
    <div class="panel panel-default" id="searchResult" style="padding:2px;">
     <div class="panel-heading crm-table-title"><B>报表明细列表</B></div>
     	<input type="hidden" id="pageNumber" name="pageNumber" value="${page.start}" />
    	<table class="table table-hover" cellspacing="0" cellpadding="0" >
              <thead>
                <th>询价单号</th>
              ...
              <s:iterator id="temp" value="quotationList" status="dl">
	              <tr class="ui-table-style1-tr2">
	                <td>${temp.quotationCode }</td>
	                <td>${temp.customerName }</td>
	                <td><s:date name="#temp.quotationTime" format="yyyy-MM-dd HH:mm:ss" /></td>
	                <td>${temp.quotationUserName }</td>
	                <td><s:date name="#temp.replyTime" format="yyyy-MM-dd HH:mm:ss" /></td>
	                <td>${temp.replyHours }</td>
	                <td>${temp.purchaseUserName }</td>
	                <td></td>
	              </tr> 
              </s:iterator>
       </table>
